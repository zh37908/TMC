...
# declare -a PRETRAINS=("No_pretrain")

for SNR in "${SNRS[@]}"; do
  for pt in "${PRETRAINS[@]}"; do
    for run in 1; do
      NAME="train_TMC_channel_snr_fewlabel_${METHOD}_${pt}_snr${SNR}_lr${LR}_run${run}"
      BASE_SAVEDIR="$ROOT/savepath/TMC_channel_snr/sunrgbd/${METHOD}/few-label/snr${SNR}/pretrain_${pt}/lr_${LR}"
      EXP_SAVEDIR="$BASE_SAVEDIR/run${run}"
      echo "=== Running FEW-LABEL (SUNRGBD) TMC_channel_snr | method=${METHOD} | pretrain=${pt} | snr=${SNR} | lr=${LR} | run=${run} ==="
