...
#         echo "=== Running TMC_channel_snr (DYNAMIC) | method=${method} | pretrain=${pt} | range=[${SNR_MIN}, ${SNR_MAX}] | lr=${lr} | run=${run} ==="
#         if [ -d "$EXP_SAVEDIR" ] && { [ -f "$EXP_SAVEDIR/model_best.pt" ] || [ -f "$EXP_SAVEDIR/checkpoint.pt" ]; }; then
#           echo "[SKIP] Exists: $EXP_SAVEDIR"
#           continue
#         fi
...
#         BASE_SAVEDIR="$ROOT/savepath/TMC_channel/sunrgbd"
#         EXP_SAVEDIR="$BASE_SAVEDIR/static_snr${snr}/pretrain_${pt}/lr_${lr}/run${run}"
#         echo "=== Running TMC_channel BASELINE | pretrain=${pt} | snr=${snr} | lr=${lr} | run=${run} ==="
#         if [ -d "$EXP_SAVEDIR" ] && { [ -f "$EXP_SAVEDIR/model_best.pt" ] || [ -f "$EXP_SAVEDIR/checkpoint.pt" ]; }; then
#           echo "[SKIP] Exists: $EXP_SAVEDIR"
#           continue
#         fi
...
#       echo "=== Running TMC_channel BASELINE (DYNAMIC) | pretrain=${pt} | range=[${BASE_SNR_MIN}, ${BASE_SNR_MAX}] | lr=${lr} | run=${run} ==="
#       if [ -d "$EXP_SAVEDIR" ] && { [ -f "$EXP_SAVEDIR/model_best.pt" ] || [ -f "$EXP_SAVEDIR/checkpoint.pt" ]; }; then
#         echo "[SKIP] Exists: $EXP_SAVEDIR"
#         continue
#       fi
