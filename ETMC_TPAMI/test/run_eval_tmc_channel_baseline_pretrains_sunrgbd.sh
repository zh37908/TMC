...
OUT_CSV="$ROOT/TMC/ETMC_TPAMI/test/results_tmc_channel_baseline_pretrains_sunrgbd.csv"
...
for pt in "${PRETRAINS[@]}"; do
  for run in "${RUNS[@]}"; do
    SAVEDIR_BASE="$ROOT/savepath/TMC_channel/sunrgbd"

    # static evals
    for SNR in "${STATIC_SNRS[@]}"; do
      SAVEDIR="$SAVEDIR_BASE/static_snr10/pretrain_${pt}/lr_1e-1/run${run}/train_TMC_channel_baseline_${pt}_snr10_lr1e-1_run${run}"
      if [ ! -d "$SAVEDIR" ]; then
