...
from models.TMC import ETMC_channel, ETMC_channel_dynamic, ce_loss
...
def get_args(parser):
    ...
    parser.add_argument("--name", type=str, default="ETMC_channel_static")
    parser.add_argument("--num_image_embeds", type=int, default=1)
    parser.add_argument("--patience", type=int, default=30)
    parser.add_argument("--savedir", type=str, default="./savepath/ETMC_channel/sunrgbd/")
...
def build_model(args):
    """Build ETMC model with optional pretrained backbone"""
    model_cls = ETMC_channel_dynamic if args.channel_mode == "dynamic" else ETMC_channel
...
def model_eval(i_epoch, data, model, args):
    ...
    for batch in data:
        loss, depth_alpha, rgb_alpha, pseudo_alpha, depth_rgb_alpha, tgt = model_forward(i_epoch, model, args, batch)
...
def cli_main():
    parser = argparse.ArgumentParser(description="Train ETMC with channel (static/dynamic) on SUN RGB-D")
    get_args(parser)
