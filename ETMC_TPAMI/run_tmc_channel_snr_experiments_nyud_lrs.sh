    ...
    for pt in "${PRETRAINS[@]}"; do
      for lr in "${LRS[@]}"; do
        for run in 1 2 ; do
          ...
          echo "=== Running TMC_channel_snr | dataset=nyud | method=${method} | pretrain=${pt} | snr=${snr} | lr=${lr} | run=${run} ==="
...
# # Dynamic SNR experiments (tmc_channel_snr)
# SNR_MIN=0
# SNR_MAX=20
# for method in "${SNR_INPUT_METHODS[@]}"; do
#   for pt in "${PRETRAINS[@]}"; do
#     for lr in "${LRS[@]}"; do
#       for run in 1 2 ; do
#         NAME="train_TMC_channel_snr_${method}_${pt}_dynamic_${SNR_MIN}_${SNR_MAX}_lr${lr}_run${run}"
#         BASE_SAVEDIR="$ROOT/savepath/TMC_channel_snr/nyud"
#         EXP_SAVEDIR="$BASE_SAVEDIR/${method}/dynamic_${SNR_MIN}_${SNR_MAX}/pretrain_${pt}/lr_${lr}/run${run}"
#         echo "=== Running TMC_channel_snr (DYNAMIC) | dataset=nyud | method=${method} | pretrain=${pt} | range=[${SNR_MIN}, ${SNR_MAX}] | lr=${lr} | run=${run} ==="
#         if [ -d "$EXP_SAVEDIR" ] && { [ -f "$EXP_SAVEDIR/model_best.pt" ] || [ -f "$EXP_SAVEDIR/checkpoint.pt" ]; }; then
#           echo "[SKIP] Exists: $EXP_SAVEDIR"
#           continue
#         fi
...
#         EXP_SAVEDIR="$BASE_SAVEDIR/static_snr${snr}/pretrain_${pt}/lr_${lr}/run${run}"
#         echo "=== Running TMC_channel BASELINE | dataset=nyud | pretrain=${pt} | snr=${snr} | lr=${lr} | run=${run} ==="
#         if [ -d "$EXP_SAVEDIR" ] && { [ -f "$EXP_SAVEDIR/model_best.pt" ] || [ -f "$EXP_SAVEDIR/checkpoint.pt" ]; }; then
#           echo "[SKIP] Exists: $EXP_SAVEDIR"
#           continue
#         fi
...
#   for lr in "${LRS[@]}"; do
#     for run in 1 2 ; do
#       NAME="train_TMC_channel_baseline_${pt}_dynamic_${BASE_SNR_MIN}_${BASE_SNR_MAX}_lr${lr}_run${run}"
#       BASE_SAVEDIR="$ROOT/savepath/TMC_channel/nyud"
#       EXP_SAVEDIR="$BASE_SAVEDIR/dynamic_${BASE_SNR_MIN}_${BASE_SNR_MAX}/pretrain_${pt}/lr_${lr}/run${run}"
#       echo "=== Running TMC_channel BASELINE (DYNAMIC) | dataset=nyud | pretrain=${pt} | range=[${BASE_SNR_MIN}, ${BASE_SNR_MAX}] | lr=${lr} | run=${run} ==="
#       if [ -d "$EXP_SAVEDIR" ] && { [ -f "$EXP_SAVEDIR/model_best.pt" ] || [ -f "$EXP_SAVEDIR/checkpoint.pt" ]; }; then
#         echo "[SKIP] Exists: $EXP_SAVEDIR"
#         continue
#       fi
