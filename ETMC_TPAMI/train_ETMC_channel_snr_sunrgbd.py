...
from models.TMC import ETMC_channel, ETMC_channel_dynamic, ETMC_channel_snr, ce_loss
...
def get_args(parser):
    ...
    parser.add_argument("--data_path", type=str, default="/home/hzhaobi/Multired/sunrgbd")
    ...
    parser.add_argument("--name", type=str, default="ETMC_channel_snr_static")
    parser.add_argument("--num_image_embeds", type=int, default=1)
    parser.add_argument("--patience", type=int, default=30)
    parser.add_argument("--savedir", type=str, default="./savepath/ETMC_channel_snr/sunrgbd/")
    parser.add_argument("--seed", type=int, default=1)
    parser.add_argument("--n_classes", type=int, default=19)
    parser.add_argument("--annealing_epoch", type=int, default=10)

    # channel options - extended with snr_model mode
    parser.add_argument("--channel_mode", type=str, choices=["static", "dynamic", "snr_model"], default="static")
    ...
    # SNR model specific parameters
    parser.add_argument("--snr_embed_dim", type=int, default=64, help="Dimension of SNR embedding")
    parser.add_argument("--snr_input_method", type=str, choices=["concat", "add", "mlp"], default="concat",
                        help="How to incorporate SNR information: concat, add, or mlp fusion")
    
    # pretrain / backbone options
...
def get_optimizer(model, args):
    ...
    return optim.SGD(model.parameters(), lr=args.lr, momentum=args.momentum, nesterov=bool(args.nesterov), weight_decay=1e-5)
...
def build_model(args):
    """Build ETMC model with optional pretrained backbone and SNR awareness"""
    if args.channel_mode == "snr_model":
        # Use SNR-aware model
        model_cls = ETMC_channel_snr
    elif args.channel_mode == "dynamic":
        model_cls = ETMC_channel_dynamic
    else:
        model_cls = ETMC_channel
...
def model_forward(i_epoch, model, args, batch, current_snr=None):
    rgb, depth, tgt = batch['A'], batch['B'], batch['label']
    rgb, depth, tgt = rgb.cuda(), depth.cuda(), tgt.cuda()

    # Handle different channel modes
    if args.channel_mode == "snr_model":
        # For SNR model, pass the current SNR as additional input
        if current_snr is None:
            # If no specific SNR provided, use the default channel_snr
            current_snr = args.channel_snr
        
        # Convert SNR to tensor and move to GPU
        snr_tensor = torch.tensor([current_snr], dtype=torch.float32).cuda()
        snr_batch = snr_tensor.expand(rgb.size(0))  # Expand to batch size
        
        # Pass SNR information to the model
        depth_alpha, rgb_alpha, pseudo_alpha, depth_rgb_alpha = model(rgb, depth, snr_batch)
    else:
        # Standard forward pass for static/dynamic modes
    ...
    return loss, depth_alpha, rgb_alpha, pseudo_alpha, depth_rgb_alpha, tgt


@torch.no_grad()
def model_eval(i_epoch, data, model, args):
    model.eval()
    losses, depth_preds, rgb_preds, pseudo_preds, depthrgb_preds, tgts = [], [], [], [], [], []
    for batch in data:
        # For evaluation, we might want to test at different SNR levels
        if args.channel_mode == "snr_model":
            # Use the configured SNR for evaluation
            current_snr = args.channel_snr
        else:
            current_snr = None
            
        loss, depth_alpha, rgb_alpha, pseudo_alpha, depth_rgb_alpha, tgt = model_forward(
            i_epoch, model, args, batch, current_snr
        )
        losses.append(loss.item())

        depth_pred = depth_alpha.argmax(dim=1).cpu().numpy()
        rgb_pred = rgb_alpha.argmax(dim=1).cpu().numpy()
        pseudo_pred = pseudo_alpha.argmax(dim=1).cpu().numpy()
    ...
    pseudo_preds = [l for sl in pseudo_preds for l in sl]
    ...
    metrics["pseudo_acc"] = accuracy_score(tgts, pseudo_preds)
...
def train(args):
    ...
    for i_epoch in range(start_epoch, args.max_epochs):
        ...
        for batch in tqdm(train_loader, total=len(train_loader)):
            # Handle different channel modes for training
            current_snr = None
            if args.channel_mode == "static":
                model.args.channel_snr = float(args.channel_snr)
            elif args.channel_mode == "snr_model":
                # For SNR model, we can vary the SNR during training for robustness
                if hasattr(args, 'snr_training_mode') and args.snr_training_mode == 'random':
                    # Randomly sample SNR from range during training
                    current_snr = np.random.uniform(args.snr_min, args.snr_max)
                else:
                    # Use fixed SNR
                    current_snr = args.channel_snr

            loss, *_ = model_forward(i_epoch, model, args, batch, current_snr)
        ...
        metrics = model_eval(i_epoch, test_loader, model, args)
        ...
        logger.info(
            ...
            f"rgb_acc: {metrics['rgb_acc']:.5f}, pseudo_acc: {metrics['pseudo_acc']:.5f}, depth rgb acc: {metrics['depthrgb_acc']:.5f}"
        )
    ...
    model.eval()
    test_metrics = model_eval(np.inf, test_loader, model, args)
    logger.info(
        f"Test: Loss: {test_metrics['loss']:.5f} | depth_acc: {test_metrics['depth_acc']:.5f}, "
        f"rgb_acc: {test_metrics['rgb_acc']:.5f}, pseudo_acc: {test_metrics['pseudo_acc']:.5f}, depth rgb acc: {test_metrics['depthrgb_acc']:.5f}"
    )
    log_metrics("Test", test_metrics, logger)


def cli_main():
    parser = argparse.ArgumentParser(description="Train ETMC with channel (static/dynamic/snr_model) on SUN RGB-D")
    get_args(parser)
