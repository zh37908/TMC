...
def get_args(parser):
    ...
    parser.add_argument("--resume", action="store_true", help="resume training from existing checkpoint", default=False)
...
def train(args):
    ...
    if args.pretrain != 'No_pretrain':
        ...
        if model_pt is not None:
            ...
            # 只加载 ResNet 主干部分（去掉 fc），并转成与 ImageEncoder_no_pretrain 相同的 nn.Sequential 结构
    ...
    # 如果提供 --resume 参数且存在 checkpoint，则恢复训练；否则从头开始
